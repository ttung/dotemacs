...
;; This variant of **ispell4.el** has been checked out for ispell4v02.  
;; Please send bug reports to tibbetts@cat.rpi.edu.
;;
;;  Brian Tibbetts, 15 February 1998
;;
;; Sun 15-Feb-1998; Brian Tibbetts
;;  'x' also activates ispell-quit to ease transition from ispell3.1
;;  per Yigal Hochberg's suggestion.
;;
;; Wed 21-Jan-1998; Bill Carpenter
;;  Fixed a bug in ispell-message.  Fixed loops with improper termination.
;; 
;; Wed 16-Apr-1997; Brian Tibbetts  
;;  Borrowed and incorporated the ispell-minor-mode from ispell3.1.xx.
;;
;; Mon 27-Jan-1997; Douglas Gray Stephens 
;;  correct mis-placed bracked in ispell-show-choices and unwind-protect
;;
;; Wed 15-Jan-1997; Douglas Gray Stephens 
;;  Modify ispell-do-look so it returns more than 10 words
;;  Modify ispell-show-choices so window is 50% of frame, 
;;  and use code from ispell.el version 2.37 (shipped with ispell 3.1.12)
;;  to enable user to select from more than 10 words.
;;  Not 100% that I have the window size set up correct.
;;
;; Thu 10-Oct-1996; Douglas Gray Stephens 
;;  Modify to always use a temporary file (this means it works with mail,
;;   which was flagged as a problem in original ispell-4.0 for NT
;;  Modify ispell-word and ispell-region so that current cursor position 
;;   is not lost.
;;  Modify ispell-still-bad, as original code would mean that if you accepted
;;   cat (assuming that cat was not in the dictionary), then the test would
;;   pass any word starting with "cat".
...
(defun ispell-cmd (&rest strings)
  (save-excursion
;;
;;  Need to change this, it kills the background ispell process
;;  (thanks, Ming-Jen Chan!)     27 June 1996 HET
;;
;; **WAS**    (ispell-sync t)
    (ispell-sync nil)
    (set-buffer (process-buffer ispell-process))
...
q, \\[keyboard-quit]	Leave the command loop.  You can come back later with \\[ispell-next]."
  ...
  ;; This line is **added** for WIN32 version of ispell--
  ...
  (save-excursion
    ...
    (let ((filename buffer-file-name)
      ...
      (unwind-protect
	  (progn
	    ;; Fri 13-Sep-1996; Douglas Gray Stephens 
	    ;;  I always want to use a temporary file
;; DGS	    (cond ((or (null filename)
;; DGS		       (find-file-name-handler buffer-file-name nil))
;; DGS
;; DGS;;
;; DGS;; Change this to use the environment's "TEMP" directory...
;; DGS;;
;; DGS;;		   (setq filename (make-temp-name "/usr/tmp/ispell"))
;; DGS		   (setq filename (make-temp-name
;; DGS				   (format "%s\\" (getenv "TEMP"))))
;; DGS;;
;; DGS		   (setq delete-temp t)
;; DGS		   (write-region (point-min) (point-max) filename))
;; DGS		  ((and (buffer-modified-p buf)
;; DGS			(y-or-n-p (format "Save file %s? " filename)))
;; DGS		   (save-buffer)))
	    (setq filename (make-temp-name
			    (format "%s\\" (or (getenv "TEMP")
					       (getenv "TMPDIR")))))
;;
		  (setq delete-temp t)
		  (write-region (point-min) (point-max) filename)
;; End of DGS's changes
...
;  The following line is added to deal with the relative offsets reported
;  by ispell4  -  BRT   9 Oct 1996
     (setq pos (+ start pos))
...
;;;autoload
;; Wed 02-Oct-1996; Douglas Gray Stephens 
;;  Added save-excursion to ensure point returned to initial position
...
(defun ispell-region (start &optional end)
  ...
  (interactive "r")
  ;; Thu 10-Oct-1996; Douglas Gray Stephens 
  ;;  added save excursion to stop cursor finishing in different place
  (save-excursion
...
(defun ispell-point (start message)
  (let ((wend (make-marker))
    ...
    ;; when you type q.
    (goto-char start)
;    (sit-for 3)                        ;useful for debugging ispell
                                        ;file positioning - BRT
...
(defun ispell-still-bad (word)
  (let ((words ispell-recently-accepted)
        ...
        (case-fold-search t))
    (while words
      ;; Fri 20-Sep-1996 10:58; Douglas Gray Stephens 
      ;; Modify, as it was matching part words 
      ;; (e.g. accept "com", therefore "comm" was ok).
      ;;  (cond ((eq (string-match (car words) word) 0)
      ;; to
      (cond ((string-equal (car words) word)
             (setq ret nil)
             (setq words nil)))
...
(defun ispell-show-choices (word message first-line)
  ...
  ;;messages window be small.  otherwise just use the other window
  (let* ((selwin (selected-window))
    ...
    (if resize
        (unwind-protect
            (progn
              (select-window w)
                ;; Wed 15-Jan-1997; Douglas Gray Stephens 
                ;;  make ispell buffer half window size
                ;;  was
    ...
    (save-excursion
      ...
      (insert first-line "\n")
      (insert
       "SPC skip; a accept; i insert; r replace; l lookup; \
q or x quit; other CHARACTER to select replacement.\n")
      (cond ((not (null message))
	     ;; Wed 15-Jan-1997; Douglas Gray Stephens 
	     ;;  Code lifted from ispell 2.37 
	     ;;  (released with ispell 3.1.12)
	     (let ((max-lines (- (window-height) 4)) ; assure 4 context lines.
		   (line 2))
	       (while (and message
		  (< (if (> (+ 7 (current-column) (length (car message))
			       (if (> count ?~) 3 0))
			    (window-width))
			 (progn
			   (insert "\n")
...
(defun ispell-command-loop (word start end message)
  (let ((flag t)
    ...
    (while flag
      (let ((command-characters '( ? ?? ?a ?i ?l ?q ?r ?x ))
	    (count ?0)
	    (skipped 0))
	...
	(let ((c (read-char))
	  ...
	  (let ((com-chars command-characters))
	    (while com-chars
	      (if (and (>= (car com-chars) ?0) (< (car com-chars) c))
			  (setq skipped (1+ skipped)))
		      (setq com-chars (cdr com-chars)))
		    (setq num (- c ?0 skipped)))
        (cond ((= c ?q)
               (throw 'ispell-quit nil))
	      ((= c ?x)  ;; added as suggested by Yigal Hochberg to ease
               (throw 'ispell-quit nil))    ;; ispell 3.1 transition
              ((= c (nth 3 (current-input-mode)))
               (keyboard-quit))
              ((= c ? )
               (setq flag nil))
              ((= c ?r)
               (ispell-replace start end (read-string "Replacement: " word))
               (setq rescan t)
               (setq flag nil))
              ((= c ?i)
               (ispell-insert word)
              ...
              ((= c ??)
               ...
               (sit-for 2))
	      ((and (>= num 0) (< num count)
                    (setq replacement (nth num message)))
...
(defun ispell-do-look (bad-word)
  (let (regex buf words)
    (cond ((null ispell-have-new-look)
           (setq regex (read-string "Lookup: " bad-word)))
          (t
	   ;; Leading "^" works only sometimes  -- peter 22-Oct-96 
           (setq regex (read-string "Lookup (regex): " (concat "^" bad-word)))))
	...
	(if ispell-have-new-look
      ...
      (goto-char (point-min))
        ;; Wed 15-Jan-1997; Douglas Gray Stephens 
        ;;  look only used to keep 10 lines, so ispell shows 3x3 wordsm
        ;;   (forward-line 10)
...
See also `ispell-look-dictionary' and `ispell-gnu-look-still-broken-p'."
  ...
  (let* ((completion-ignore-case t)
         ...
         (string (buffer-substring-no-properties bow (point)))
    ...
    (cond
     ...
     ((and (not (string-equal completion ""))
      ...
      (if (not (equal 1  (length ispell-lookup-completions-alist)))
	...
	(message "Making completion list...done")
 	(if (setq choice 
	      (completing-read "Ispell complete: " (mapcar 'list list) 
			       nil t completion))
	    (progn
	      (delete-region bow (point))
...
;;;autoload
(defun ispell-complete-word-in-dictionary (dictionary &optional interior-frag)
  "Run ispell-complete-word using an alternate dictionary"
  (interactive "fDictionary: ")
  (let ((ispell-look-dictionary  (expand-file-name dictionary))
	(ispell-lookup-last-word nil))
  ...
  (save-excursion
    (let (non-internal-message
      ...
      (let* ((cite-regexp               ;Prefix of inserted text
        ...
        ;; Check the body.
	;; Modified by Bill Carpenter to fix loops in original dist.
        (while (and (< (point) limit) continue (not (eobp)))
            ;; Skip across text cited from other messages.
            (while (and (looking-at (concat "^[ \t]*$\\|" cite-regexp))
 					  (not (eobp))
                        (< (point) limit))
	      ;; End of Bill's modifications.
            (forward-line 1))
          (if (< (point) limit)
